...
import com.lovdmx.control.httpVo.excelExprot.gatewayDevice.GatewayDeviceDetailsVo;

/**
 * 
 * Copyright: Copyright (c) 2019 LanRu-Caifu
 * 
 * @ClassName: ExcelExportGatewayDevice.java
 * @Description: Excel导出 网关设备信息
 *
 * @version: v1.0.0
 * @author: syz
 ...
 *
 */
public class ExcelExportGatewayDevice extends ExcelExport<GatewayDeviceDetailsVo> {

	public ExcelExportGatewayDevice(HSSFWorkbook wb, String[] headList) {
		super();
		this.wb = wb;
		this.headList = headList;
	}

	public ExcelExportGatewayDevice(HSSFWorkbook wb, String[] commHeadList, List<Object> commHeadDataList,
			String[] headList) {
		super();
		this.wb = wb;
	 ...
	 */
	public void createSheet(String sheetName, String title, List<GatewayDeviceDetailsVo> dataList) {
		...
		int b = 2;
		// 数据导入单元格
		for (GatewayDeviceDetailsVo gatewayDevice : dataList) {
			...
			for (int i = 0; i < headList.length; i++) {
				...
				cell1.setCellStyle(cellStyle);
				if (i == 0) {
					cell1.setCellValue(gatewayDevice.getUploadTime());
				} else if (i == 1) {
					cell1.setCellValue(gatewayDevice.getTemp());
				} else if (i == 2) {
					cell1.setCellValue(gatewayDevice.getHumi());
				} else if (i == 3) {
					cell1.setCellValue(gatewayDevice.getSmokeAlarmSate());
				}
			}
			b++;
		}
