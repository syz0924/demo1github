...
/**
 * CRC16 校验算法: 校验(CRC)占用两个字节,包含了一个 16 位的二进制值。CRC 值由传输设备
 * 计算出来,然后附加到数据帧上,接收设备在接收数据时重新计算 CRC 值,然后与 接收到的 CRC
 * 域中的值进行比较,如果这两个值不相等,就表示数据传输发生了错 误。生成一个 CRC 的流程为: ---------------------
 * 第一步:预置一个 16 位寄存器为 0FFFFH(全 1) ,称之为 CRC 寄存器。
 * 第二步:把数据帧中的第一个字节的 8 位与 CRC 寄存器中的低位字节进行异或 运算,结果存回 CRC 寄存器。
 * 第三步:将 CRC 寄存器向右移一位,最高位填以 0,最低位移出并检测。 
 * 第四步:如果最低位为 0:重复第三步(下一次移位);如果最低位为 1:将 CRC 寄存器与生成多项式(CRC16 多项式对应值 A001H)进行异或运算。 
 ...
 * 
 * @ClassName: CRC16Util.java
 * @Description: CRC16校验算法实现
 *
 * @version: v1.0.0
 * @author: syz
 * @date: 2019年1月18日 上午11:28:33
 *
 */
public class CRC16Util {
	...
	public static String getCrc(int res) {
		...
		System.err.println("BLUEDATA crc ---- : " + substring + "  " + substring1);
